...
class mbar_pmf(object):
    ...
    def __init__(self, val_kn, val0_k, K_k, temperature, tol=1.0e-8, u_kn=None, u_kln=None):
        ...
        self.val_kn = np.zeros((self.K, self.N_max), float)
        mask = np.arange(self.N_max) < np.array(self.N_k)[:,None]
        self.val_kn[mask] = np.concatenate(val_kn)
        self._u_kn = np.zeros_like(self.val_kn, float)

        if u_kn is not None:
            self._u_kn0 = u_kn * self.beta
        else:
            self._u_kn0 = np.zeros_like(self.val_kn, float)
        ...
        for k in range(self.K):
            for n in range(self.N_k[k]):
                for l in range(self.K):
                    ...
                    self._u_kln[k,l,n] += self._u_kn0[k,n] + self.beta * (self.K_k[l]/2.0) * dval**2
    ...
    def get_pmf(self, val_min, val_max, nbins, u_kn=None, **kargs):
        ...
        bin_kn = np.digitize(self.val_kn, bin_edges) - 1
        # for i in range(nbins):
        ...
        # results = self.mbar.computePMF(_u_kn, bin_kn, nbins, **kargs)
        # f_i = results['f_i']
        # df_i = results['df_i']
